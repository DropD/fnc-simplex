...
The same technique was used to measure runtime, however instrumentali\mytodo{references}zation was not used.
...
Of our own optimization techniques, blocking with an additional copy (swap) step showed to be the most efficient, both non-auto-vectorized (Fig) and auto-vectorized (Fig).

Our further goal was to find the optimal block sizes by varying unrolling width and concurrent line elimination count, which \mytodo{incidentally} turned out to be \mytodo{our originally picked sizes,} $2\times 4$.
...
\dtodo{AVX (was macht das hier so ganz allein?)}
