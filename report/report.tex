...
\begin{eqnarray*}
    ...
    x \geq 0.
\end{eqnarray*}
Any linear problem can be transformed into standard form \rtodo{citation needed? -> Yes, cite the Weismantel script or if possible the book}. Considering this we chose to restrict our solver to standard form problems, since supporting other formulations would have lead to more effort being put into transforming the problem into standard form (effectively implementing a domain specific language).
...
yet they do not match the fastest auto-vectorised version (Fig).
...
We were able to realize a speedup of up to 25\% for small problems and up to 15\% for large ones,
albeit limited to feasible standard maximization LPs.

\mytodo{say something more? feels so little here}
...
\mytodo{Make sure you define every acronym you use, no matter how
