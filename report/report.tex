...
\begin{itemize}
    ...
    \item{SoPlex \cite{soplex}, free for educational use, developped at Zuse Institute Berlin.}
\end{itemize}
\rtodo{cite original danzig paper?}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Background: The Simplex Algorithm}\label{sec:background}
\rtodo{cite handouts from opti.}
...
\mypar{Standard form Linear Problems} \mytodo{explain standard form, mention restriction to standard form}
...
\begin{enumerate}
    \item initialize the tableau $T \in \mathbb{R}^{(m+1) \times (n+m+1)}$
        \[ T = 
            ...
            \right]
        \]
        And create a set of column indices to keep track of the currently ``active'' columns (also caled the basis). 
...
\mypar{Cost Analysis}
The cost of the overall algorithn depends highly on the coefficients of the problem,
...
\mypar{Cache control}
Improvement of cache access attempted through intrinsics for nontemporal assignment in order to avoid polluting the cache with updates on the active basis elements and
...
very short summary of the section. The typical structure follows.}
 \ptodo{Specify the platform (processor, frequency, cache sizes)
...
All experiments were conducted on a Core i5-3360M (Ivy Bridge) running GNU/Linux and 32kB L1 cache, 256kB L2 cache and 3MB L3 cache, all at 64 bytes line size.
...
We used Gurobi\mytodo{references}, SoPlex, GLPK and CPPLEX, with the last one being dropped from testing because of numerical instability for large problems.
...
As shown in Fig.~\ref{fig:res_cachecontrol}, nontemporal assignment showed a mostly positive but small effect on the swapping implementations,
